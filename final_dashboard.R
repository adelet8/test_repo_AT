...
ui <- dashboardPage(
  ...
  # Create a sidebar on the left-hand side
  dashboardSidebar(
    sidebarMenu(menuItem("Instructions", tabName = "Instructions", icon = icon("info")),
  ...
  dashboardBody(
    tabItems(
      tabItem("Instructions",
        fluidRow(box(width = 12, title = "Dashboard Overview",
...
## Text Output for dashboard overview
 output$textOutput <-renderUI({
   HTML(paste("Thank you for choosing to use our Data Center Temperature Monitoring Dashboard! <br> <br>",
               "Instructions for use: <br> <br>",
               "1. Upload your dataset (or real-time data) using the sidebar Update CSV feature. <br>",
               "2. Once your dataset is uploaded, you should notice the plots and data center layout update to your data. <br>",
               "3. Select Metric: In the Controls block, you will see a drop down menu. Select the temperature metrics you would like to analyze. <br>",
               "4. Select Components: Using the interactive Data Center Hierarchy, you are able to select the scope of analysis. Do you want to look at the whole data center, or a single GPU? By selecting the component, and therefore level of hierarchy, the time plot will update to show the average temperature data for that grouping of components. <br>",
               "5. Select Specification Limits: Select the check box and adjust the slide bars on the left-hand side of the dashboard. This controls the lower and upper temperature specification limits, respectively. When you click on the check box, you will enable those limits to be plotted on the time chart. They will also be plotted on the histogram below the Hierarchy. <br> ",
               "6. Statistical Process Control and Failure Statistics: Enabling the specification limits will also enable the failure percentage value to be displayed in the Control Specifications block. This qualitatively shows how much of the data falls outside specification, which is also an indication of the percentage of GPUs with an elevated risk for failure. <br>",
               "7. The Histogram: This plot will also update with the component and level of scope selected in the data center layout. However, it shows a plot of all of the data within the selected scope, which is different from the time plot (which looks at average temperatures). This histogram is plotted against a normal distribution plot for shape reference. <br>",
               "8. Control Plots: The bottom section of the dashboard provides the Average Plot and Standard Deviation Plot for specific components of interest over time. This is very similar to the interactive time plot above, but allows for greater resolution. The blue shaded area represents the range of the 3-sigma control limits. <br> <br>",
               "When you are ready to begin, select the tab for 'SPC' in the top left-hand part of the Dashbord.<br> <br>",
              "Very Respectfully, <br> <br>",
              "The Six Sigma Warriors <br> <br>",
              "     Adele Thompson <br>",
              "     Brandy Kinnunen <br>",
              "     Juan Rodriguez <br>",
              "     Michael Fizdale",
  ...
  output$ts_plot <- renderPlotly({
    ...
    g <- ggplot(pdat, aes(t, y)) +
      ...
      { if (isTRUE(input$show_specs)) geom_point(data = oos_high, aes(t,y), color = "red3", size = 0.9)}+
      { if (isTRUE(input$show_specs)) geom_point(data = oos_low, aes(t,y), color = "blue", size = 0.9)}+
  ...
  output$caps <- renderUI({
    ...
    tagList(
      ...
      tags$br(),
      tags$small("- If Cp or Cpk >= 1: the process is stable, meaning it operates within the expected specification limits."),
      tags$br(),
      tags$small("- If Cp or Cpk < 1: the process is unstable, meaning it does not operate within the expected specification limits."),
      tags$br(),
      tags$small("- The % Out of Spec indicates the percentage of datapoints which fall outside of the upper and lower specification limits.")
    )
  })
